...
val downloaderModule = module {
    ...
    single {
        val definedPaths = get<DefinedPaths>()
        ir.amirab.downloader.downloaditem.ytdlp.YtdlpProcessManager.init {
            val platform = ir.amirab.util.platform.Platform.getCurrentPlatform()
            val exeName = if (platform == ir.amirab.util.platform.Platform.Desktop.Windows) "yt-dlp.exe" else "yt-dlp"
            definedPaths.systemDir.resolve(exeName).toString()
        }
        ir.amirab.downloader.downloaditem.ytdlp.YtdlpDownloader {
            ir.amirab.downloader.downloaditem.ytdlp.YtdlpProcessManager.getExePath()
        }
    }
    single {
        com.abdownloadmanager.shared.downloaderinui.ytdlp.YtdlpDownloaderInUi(get(), get())
    }
    single {
        DownloaderInUiRegistry().apply {
            add(get<com.abdownloadmanager.shared.downloaderinui.ytdlp.YtdlpDownloaderInUi>())
    ...
    single {
        DownloaderRegistry().apply {
            add(get<ir.amirab.downloader.downloaditem.ytdlp.YtdlpDownloader>())
